...
| `2026-09-24-demand-batch-took-five-hours.md`       | 2026-09-24                          | `other`           | Four demand letters took 5 h 14 min where the drafting itself takes about 25 minutes each: the summary step could not run live (non-streaming call, `llm.py:441`) so it queued in Batch, documents took 100 minutes off the seat, and inputs were found incomplete after paid stages. Source: each matter's `run.json`; the note itself. |
