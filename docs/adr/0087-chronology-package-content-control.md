...
**Date-of-service completeness (2026-09-24), a check, not yet a gate.** Gates 1 and 4 cannot see a visit that never became a claim: a record holding six visits and a chronology naming five passes both. The runner's `dos_check` stage (`medchron/dos_check.py`, after `coverage_gate`) takes every billed date of service (MEDICAL_BILL and LEDGER line items, payments and adjustments dropped) and places it in the chronology, before the incident (summarized by design), explained by a person (`medchron explain-date`), billed with no record in the file (a records gap, since billing-only dates stay out of entries by design), a **provider unmatched** visit (entries carry the date, none names the billing provider, and the medical records do name it, so one of two providers seen that day may be missing; reported, never held, because provider names differ between bills and records. A billing name no record page uses, such as a physician group invoicing for hospital care, cannot appear in a record-built chronology, so its covered date counts as in the chronology), or a **missed visit** (a record page carries the date and no entry does). The unit is the (date, provider) pair, and a missing billing extraction or one with no bills is reported as NOT MEASURED rather than as a clean pass. It writes `dos_report.json` on every run and holds only when the firm authors `levers.dos_check: hold`. It is not registered in `runtime-controls.yaml` because by default it measures and does not halt. Its reach is bounded by the bills in the file: on the first delivered run it measured (2026-09-24) it saw 11 billed visits against 196 entries: 9 in the chronology, 1 before the incident, 1 billed with no record in the file, 0 missed. Five same-day name mismatches it first reported were two billing companies that no record page names.
...
**Where the sets come from, and why not from the downloads.** `medchron/covered.py` reads the coverage gate's own accounting: cited in the delivered document, a byte-duplicate of something cited, in the authored billing-chart set, or excluded by an authored name rule counts as covered; contentless or `compose_skip`, a composer "nothing extractable", a failed retrieval, or a documented orphan counts as uncovered. The download log was rejected as the source: its `ok` flag means bytes arrived and hashed, so a glyph-junk scan, an orphan and a vendor order form are all `ok`. Sourcing coverage from that flag would mark a contentless scan covered and drop those records from every later update, permanently and silently, inside a litigation document. Where the two sets disagree about a document, uncovered wins: re-reading is recoverable, a dropped record is not.

**The falsifier.** The broker refuses a payload whose sets overlap, repeat an id, or whose total disagrees with the count the run pulled, because a mismatch is a stage having dropped rows between the coverage gate and this record, which is the failure species the coverage gate itself exists for. A job with no record reads as null rather than as an empty set: nothing-was-covered and nobody-wrote-it-down send an update in opposite directions, and the skill refuses to submit on null rather than approximating.

**Reaches the seats on the next reprovision.** A chronology delivered before this release carries no record, so its update needs a Captain-run reconstruction on the seat: `docs/runbooks/operator/routine-lifecycle.md`.
