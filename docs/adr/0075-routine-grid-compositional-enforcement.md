...
**Two portal defects fixed with it, both in the level reader (`src/lib/operator/entitlement-compiler.ts`).** A routine with no send action class resolved to flag-only, so five prepare-and-route work-product routines and the internal-record chronology read one or two levels below the agreement on the client's Settings page. The first correction returned the grid's authored tier instead, which is the worse error in the other direction: it would claim "Handles it" from a historical field while the seat held `internal_write: refused`. The resolved level is now the authored tier wherever the routine may write at all, because `internal_write` has two outcomes at the enforcement point and both `autonomous` and `draft_for_review` EXECUTE the write (overlay `plugins/hermes-smd-trust/enforce.py`, whose own comment records a `create_memo` landing on a real matter while its audit row read "draft"); an absent or refused class reads as "not currently authorized", and a level the Machine did not confirm is marked unconfirmed per row rather than asserted. A first pass capped such a row at prepare-and-route whenever the class was not `autonomous`, which invented a third state the gate does not have and understated six A&P routines, A&P being a seat that authors `draft_for_review` and acts. The Duties grid, Settings and the Activity feed now share one client vocabulary (`src/lib/portal/operator/tier-language.ts`) instead of three copies.
