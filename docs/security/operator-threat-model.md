...
- **Principal-identity Gmail send (as `scott@`/`team@`/`smdurgan@`): double-locked** — hard-banned at the tool layer (overlay `shared/action_classes.py` `BANNED_TOOLS`, _"never send as Scott"_) **and** Wave-B-deferred. Strong. Unchanged by ADR 0089.
- **Staff send-as on a client's M365 seat (ADR 0089, 2026-09-21): approval-bound, broker-owned.** The Operator mailbox holds Exchange Send As on each `scope.staff_send_as` address, and the broker sets a From only on a row that staff member approved by email, bound by digest, consumed once, with a Sent Items forgery guard. Residual, stated: Send As means any credential able to call `sendMail` on that mailbox can impersonate staff. Guards: the two-app split (the agent never holds the send app), a boot-smoke probe that fails a send-as seat whose agent-held read app carries `Mail.Send` (`operator/templates/msgraph-read-app-cannot-send-probe.py`), and identity-joinable audit rows on every send so `operator/bin/reconcile-sends.py` flags any Sent Items message the broker did not send.
- **Device-sender reply redirect (`scope.device_senders`, 2026-09-25): authored pair, broker-rechecked.** A reply to an authored device (an office scanner nobody reads) goes to its authored `replies_to` instead of back to the device. The only widening of the reply lock, and a closed one: the validators require the device on `inbound_allow_from` and the target on `scope.admins`; the broker (`operator/workspace_broker/msgraph_ops.py` `_device_redirect`) re-derives the pairing from its own customer.yaml against the sender it fetches, so a caller naming any other `to` is refused and audited. The default wire path (`msgraph_redirect.py`) replaces the draft's recipients and reads them back before sending. Residual, stated: the draft is written on the agent-held read app, so an agent could PATCH it in the one-request window between read-back and send, the same pre-existing property as replying to a mutable message.
...
3. **Principal-identity send is hard-banned** `[verified — overlay shared/action_classes.py BANNED_TOOLS]`. "Never send as Scott" is a code-level floor, not a config choice. ADR 0089's staff send-as does not relax it: it is a separate, authored, per-message-approved lane on a client's own tenant, never an autonomous send as a principal.
