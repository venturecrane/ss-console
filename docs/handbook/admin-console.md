...
- **Obligations** (`/admin/obligations`) is the fleet-wide answer to "what's on our plate": every open obligation across every client, grouped by client, filterable by client, kind and urgency. Rows arrive two ways - imported each reconciler pass from the systems that already know (client-labelled GitHub issues, open seat conditions, change requests), and captured from client letters with a verbatim quote. It is deliberately READ-ONLY: an obligation reaches `verified` only when a recorded CI run probed the real surface, so a "mark done" control here would be the self-certified closure the register exists to prevent ([ADR 0088](/admin/playbook/obligation-register)).
