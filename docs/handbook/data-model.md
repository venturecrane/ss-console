...
| `fleet_alert_state` | The pager's ledger: one open or resolved row per seat and condition, so a condition pages once when it opens and once when it clears rather than every minute. Rebuilt by migration 0107 (the gateway-loop conditions) and 0109 (refused or unsent escalations: the 2026-08-19 incident where a seat's deadline escalator was refused by its own gates for four days and nobody was paged). Since migration 0115 it also holds the `edge_down` row for the web Worker's own outside-in probe, keyed by the host name rather than a seat. |
