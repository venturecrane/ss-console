...
- `operator/bin/lib/decommission_backends.py`: the real backends and `backends_from_env`
- `operator/bin/lib/decommission_archiver.py`: `EvidencePacketArchiver`, step 07
...
| 7   | `07_compliance_archive`    | `EvidencePacketArchiver` (`bin/lib/decommission_archiver.py`): runs the evidence builder over the step-02 `machine-snapshot-{date}.sqlite` (opened read-only) for the whole ledger, with the console's newest `audit_head_history` head as the truncation pin, and writes the signed `compliance-packet-{date}.tar.gz` under `archive_root/{slug}/`. The manifest carries the path, file count, bytes, manifest hash and signature check read back from disk. No snapshot, a pin the ledger lacks, or a packet that does not verify fails the step. | A packet already archived today is re-read and reported `skipped: true, reason: packet_already_archived`; unwired, the stub reports `skipped` and `--live` refuses. |
...
| `ComplianceArchiver`    | `EvidencePacketArchiver`            | Builds the signed evidence packet from the step-02 snapshot, then reads it back: file count from the tarball, manifest hash recomputed, `manifest.sig` verified against the staged key. Not a deletion, but a live run destroys the Machine the packet is evidence about, so an unwired archiver refuses `--live` like the rest. | `EVIDENCE_PACKET_SIGNING_KEY_B64` + `CLOUDFLARE_API_TOKEN` (chain pin) |
...
| `test_noop_compliance_archiver_skips_and_writes_nothing` | The archiver stub reports `skipped: true` and writes nothing; `test_cli_live_refuses_when_only_the_archiver_is_unwired` proves a live run with every deletion wired still exits 5 naming it. |
...
- ~~Replace `InMemoryComplianceArchiver`~~: done 2026-09-25 (`EvidencePacketArchiver`). The packet stays on the Captain's disk under `--archive-root`; copying it to a cold-storage bucket is not built.
