---
description: Post-generation validator rubric. Documents all 24 rules (R1–R15 chrome, R16–R24 IA + pattern + a11y). Applies to any HTML — generated by product-design, other generators, or hand-authored. validate.py implements; this file specifies.
---

# Validation Rubric

`validate.py` enforces 24 rules on any HTML (generated by `product-design` or shipped code) and on the spec itself. This document is the authoritative rubric; `validate.py` implements it.
...
The validator exits 0 (pass) when violation_count is 0, and 1 (fail) otherwise. Severity affects retry behavior in the generation pipeline:
...
- name: Validate navigation (chrome + IA)
  run: |
    for file in .design/designs/**/*.html; do
