...
def rows_from_sqlite(path: str) -> list[dict]:
    ...
    try:
        ...
        cur = conn.execute(f"SELECT {', '.join(cols)} FROM audit_log ORDER BY rowid")  # noqa: S608 - column names are the CHAIN_COLUMNS constant plus two literals; no input reaches the text
