...
# The guards (lib/firm-config-currency.sh) die on a missing engagements checkout
# and on a firm config that is not engagements origin/main, so no guess reaches R2.
# The checkout check runs first and unconditionally: a missing tree cannot say
# whether this seat authors a firm config at all.
...
# ---------- Step 6b-agentmail: AgentMail channel creds (ss#2258, per-seat) -----
# The block lives in lib/stage-agentmail.sh (sourced, same shell) since
# 2026-09-17; see that file's header for why.
# shellcheck source=lib/stage-agentmail.sh
source "${BIN_DIR}/lib/stage-agentmail.sh"
...
# The block lives in lib/stage-smokeball.sh (sourced, same shell) since ss#2425;
# see that file's header for why.
# shellcheck source=lib/stage-smokeball.sh
source "${BIN_DIR}/lib/stage-smokeball.sh"
