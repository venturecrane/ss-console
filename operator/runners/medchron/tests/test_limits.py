...
def _limits(**kw) -> Limits:
    base = dict(cap_usd=150.0, monthly_budget_usd=800.0, usd_per_scanned_page=0.02, usd_per_audit_claim=0.05)
    base.update(kw)
    return Limits(**base)
...
def test_one_matter_may_consume_the_whole_cycle_allowance() -> None:
    ...
    assert (
        _hold(lim.check_before_first_paid, pages=15_001, projected_usd=1.0, spent_usd=0.0, stage="vision").setting
...
def test_no_page_gate_fires_when_the_cycle_state_is_absent() -> None:
    ...
    """
    _limits().check_before_first_paid(pages=1_000_000, projected_usd=1.0, spent_usd=0.0, stage="vision")
...
def test_the_hold_sentence_names_the_cycle_in_prose_not_a_range() -> None:
    ...
    `allowance_cycle_label` and the sentence becomes "remain in
    2026-09-15..2026-10-15's allowance"."""
    lim = _limits(allowance_remaining_pages=40, allowance_cycle_label="the cycle ending Oct 14")
    hold = _hold(lim.check_before_first_paid, pages=41, projected_usd=1.0, spent_usd=0.0, stage="vision")
    assert "40 pages remain in the cycle ending Oct 14's allowance" in hold.reason
    # The unanchored case is unchanged: YYYY-MM, as every seat had before.
    lim = _limits(allowance_remaining_pages=40, allowance_month="2026-09")
    hold = _hold(lim.check_before_first_paid, pages=41, projected_usd=1.0, spent_usd=0.0, stage="vision")
    assert "40 pages remain in 2026-09's allowance" in hold.reason
    # And with neither, it never says "month" -- the seat may be on a cycle.
...
def test_the_cap_holds_on_the_projection_and_on_spend_already_past_it() -> None:
    ...
    assert _hold(lim.check_before_paid, projected_usd=5.01, spent_usd=0.0, stage="vision").setting == "per_job_cap_usd"
    assert _hold(lim.check_before_paid, projected_usd=0.0, spent_usd=5.0, stage="audit").setting == "per_job_cap_usd"
...
def _every_reason() -> list[LimitHold]:
    ...
    out = [
        _hold(
            _limits(allowance_remaining_pages=1, allowance_month="2026-09").check_before_first_paid,
            pages=9_999,
            projected_usd=0.0,
            spent_usd=0.0,
            stage="vision",
        ),
        _hold(_limits(monthly_budget_usd=1.0).check_before_paid, projected_usd=99.99, spent_usd=0.0, stage="vision"),
        _hold(_limits(cap_usd=1.0).check_before_paid, projected_usd=99.99, spent_usd=0.0, stage="audit"),
        _hold(_limits(monthly_budget_usd=1.0, month_cents_used=100).check_each_call, spent_usd=12.34, stage="vision"),
        ...
        _hold(
            _limits(monthly_budget_usd=1.0).check_before_paid,
            projected_usd=99.99,
            spent_usd=0.0,
            stage="vision",
            batch=True,
        ),
        _hold(_limits(cap_usd=1.0).check_before_paid, projected_usd=99.99, spent_usd=0.0, stage="vision", batch=True),
    ]
    assert len(out) == 7
    return out
...
def test_the_settings_named_are_the_three_the_firm_and_the_seat_author() -> None:
    assert {h.setting for h in _every_reason()} == {
        limits_mod.ALLOWANCE_SETTING,
        limits_mod.BUDGET_SETTING,
        limits_mod.CAP_SETTING,
    }
