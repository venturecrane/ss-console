...
def test_a_control_is_verified_against_a_different_institution(tmp_path: Path) -> None:
    ...
    VA medical center exhibit's first pages, which carry the same VA
    medication list; the verifier said SUPPORTED, truthfully, and one such
    control voided a round that had discriminated 278 times. The other exhibit
    must belong to a different institution when the matter has one."""
    ...
    pdfs = {
        1: ex(1, "Example VA Medical Center"),
        2: ex(2, "Example VA Dental Clinic, Jane Doe"),
        3: ex(3, "Riverside Chiropractic"),
        19: ex(19, "Example VA Medical Center, Pharmacy (Roe)"),
    }
    assert institution(pdfs[19]) == "example va" == institution(pdfs[2])
    assert control_exhibit(19, pdfs) == 3, "not Ex1: same institution"
    assert control_exhibit(3, pdfs) == 1, "the chiropractor controls against the first VA exhibit"
