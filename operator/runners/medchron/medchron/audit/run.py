...
class Round:
    ...
    def image_verdict(self, c: dict[str, Any], pdfs: dict[int, Path]) -> tuple[dict[str, Any], list[int] | None]:
        ...
        if v["verdict"] in ("PARTIAL", "UNSUPPORTED"):
            ...
            if wide != c["pages"]:
                try:
                    wimgs = [
                        img_block(p) for p in (render(pdfs[ex], pg, self.paths.pages, f"ex{ex}") for pg in wide) if p
                    ]
                    wv = VF.verify_image(
                        self.doorway,
                        self.model,
                        c["claim"],
                        wimgs,
                        f"Exhibit {ex} p.{min(wide)}-{max(wide)}",
    ...
    def control_for(
        self, c: dict[str, Any], pdfs: dict[int, Path], index: PageIndex | None, doc_sha: str, ts: str
    ) -> None:
        with self._lock:
            self.counter["n"] += 1
    ...
    def report_line(self, c: dict[str, Any], v: dict[str, Any]) -> None:
        ...
        self.log(
            f"  {mark} Ex{c['exhibit']:<3} p.{(c['page_spec'] or '1'):<8} {str(v.get('verdict')):<15} "
            f"{str(v.get('note'))[:64]}"
        )
        ...
        try:
            ...
            CL.append_row(
                self.paths.results,
                {
                    ...
                    "claim": c["claim"][:500],
                    **ERROR,
                    "note": f"worker exception: {str(exc)[:200]}",
                    "mode": "image",
                    "doc_sha": doc_sha,
                    "ts": ts,
                },
        ...
        for c in cl["claims"]:
            try:
                ...
                rec = {
                    ...
                    "claim": c["claim"][:500],
                    "mode": "text",
                    "text_then_image": False,
                    "text_verdict": tv["verdict"],
                    "text_note": tv["note"],
                    "supporting_pages": tv["supporting_pages"],
                    "window": cl["pages"],
                    "anchors": c["anchors"],
                    "anchors_found": AN.found_on(c["anchors"], block),
                    "doc_sha": doc_sha,
                    "ts": ts,
                }
    ...
    def plan_text(self, live: list[dict[str, Any]], index: PageIndex) -> tuple[list[dict], list[dict], dict[str, int]]:
        ...
        for c in live:
            ...
            if win is None:
                cls = next(
                    (
                        index.classify(c["exhibit"], p)[0]
                        for p in c["pages"]
                        if index.classify(c["exhibit"], p)[0] != "native"
                    ),
                    "span>cap",
                )
        ...
        for ex in sorted(per_ex):
            ...
            for pages, keys in AN.build_clusters(
                [(c["key"], c["window"]) for c in cs], cap=WINDOW_CAP, floor=3, index=index, exhibit=ex, npages=np_
            ):
    ...
    def execute(self) -> int:
                ...
                oor.append(
                    {
                        **c,
                        "pages": pgs,
                        "bad_pages": bad,
                        "doc_pages": npages[c["exhibit"]],
                        "verdict": "PAGE_OUT_OF_RANGE",
                        "kind": "real",
                        "unsupported_assertions": [],
                        "contradictions": [],
                        "note": f"cites page(s) {bad} in a {npages[c['exhibit']]}-page document",
                        "mode": self.mode,
                        "doc_sha": doc_sha,
                        "ts": ts,
                    }
        ...
        if orphans:
            self.log(
                f"!! DOUBLE-SWEEP GUARD: {len(orphans)} prior row(s) audited against THIS body under keys no longer "
    ...
    def report(self) -> int:
        ...
        self.log(
            f"CONTROLS: {len(ctl)} run, {len(ctl_bad)} wrongly SUPPORTED -> "
