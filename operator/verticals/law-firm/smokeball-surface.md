...
**The `x-api-key` (recorded 2026-09-15, ss#2424; the 403 root cause that survived six weeks because this paragraph did not exist).** The API key is a separate credential from the app's `client_id`/`client_secret`, and it is **account-scoped per (region, environment)**, not per app: one Smokeball account holds six distinct keys for six gateways (US, AU, UK; production and staging each; AWS `us-east-1` / `ap-southeast-2` / `eu-west-2`). Every app on the same gateway sends the same key. Where it lives: `console.smokeball.com`, **left-nav "API" section**, on the tab for the environment ("US API Key", click to reveal). It is **not** on the Apps credentials card, which shows only the client id and secret; copying from that card is exactly how an OAuth client secret ended up in `SMOKEBALL_PROD_API_KEY` twice (2026-08-18). Shape: 40 characters, mixed-case alphanumeric (an AWS API Gateway key); a Cognito client secret is 51 or 52 lowercase characters and digits, which is why the provisioner's stage-time shape gate (ss#2423, `operator/bin/lib/secret_shape.py`) can tell them apart. Rotation is self-serve in the same API section (Actions, Rotate Key) and takes effect for every app on that gateway at once. The usage plan is the standard partner allocation, **burst 5 / rate 5 requests per second**; exceeding it returns **429**, never 403, and increases are rarely granted. Smokeball's own docs disagree on whether the plan is enforced per endpoint or globally; the connector's 429 backoff treats it as global. The docs' example `x-api-key` value (a base64 of `client_id:cl`) is a placeholder, not a format hint.
